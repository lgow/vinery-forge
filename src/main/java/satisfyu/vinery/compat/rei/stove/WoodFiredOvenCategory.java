...
public class WoodFiredOvenCategory implements DisplayCategory<WoodFiredOvenDisplay> {
	...
	public List<Widget> setupDisplay(WoodFiredOvenDisplay display, Rectangle bounds) {
		...
		widgets.add(Widgets.createLabel(new Point(bounds.x + bounds.width - 5, bounds.y + 5),
				Component.translatable("category.rei.cooking.time&xp", df.format(display.getXp()),
						df.format(cookingTime / 20d))).noShadow().rightAligned().color(0xFF404040, 0xFFBBBBBB));
		int move = 20;
		int moveDown = 10;
		widgets.add(Widgets.createArrow(new Point(startPoint.x + 24 + move, startPoint.y + 8 + moveDown))
				.animationDurationTicks(cookingTime));
		widgets.add(
				Widgets.createResultSlotBackground(new Point(startPoint.x + 61 + move, startPoint.y + 9 + moveDown)));
		widgets.add(Widgets.createSlot(new Point(startPoint.x + 61 + move, startPoint.y + 9 + moveDown)).entries(
	...
	public Renderer getIcon() {
		return EntryStacks.of(VineryBlocks.WOOD_FIRED_OVEN.get());
	}
