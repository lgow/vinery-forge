...
public class FermentationBarrelCategory implements DisplayCategory<FermentationBarrelDisplay> {
	...
	public Renderer getIcon() {
		return EntryStacks.of(VineryBlocks.FERMENTATION_BARREL.get());
	}
	...
	public List<Widget> setupDisplay(FermentationBarrelDisplay display, Rectangle bounds) {
		...
		widgets.add(Widgets.createArrow(new Point(startPoint.x + 54, startPoint.y - 1))
				.animationDurationTicks(AgingBarrelBlockEntity.COOKING_TIME_IN_TICKS));
		widgets.add(Widgets.createResultSlotBackground(new Point(startPoint.x + 90, startPoint.y)));
		widgets.add(Widgets.createSlot(new Point(startPoint.x + 90, startPoint.y)).entries(
