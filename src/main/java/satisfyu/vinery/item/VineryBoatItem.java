...
public class VineryBoatItem extends Item {
	private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);

	private final String woodType;

	public VineryBoatItem(Properties p_41383_, String woodType) {
		super(p_41383_);
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
		...
		if (hitResult.getType() == HitResult.Type.MISS) {
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				...
				summonBoat(new VineryBoat(level, x, y, z), level, hitResult, player, boat);
			}
		}
		return InteractionResultHolder.pass(boat);
	}
